...
HRESULT C_SkinObj::CreateVertexBuffer()
{
	HRESULT hr = S_OK;
...
void    C_SkinObj::ConvertSkin(std::vector<PNCTIW_VERTEX>& list)
{
	int iBaseVertex = 0;
	int iBaseIndex = 0;
	int iBaseOffset = 0;
...
HRESULT C_SkinObj::CreateVertexData()
{
	HRESULT hr = S_OK;
...
HRESULT C_SkinObj::CreateIndexData()
{
	HRESULT hr = S_OK;
	
	return hr;
}
bool	C_SkinObj::Load(T_STR loadfile,ID3D11Device* pd3dDevice,ID3D11DeviceContext* pContext)
{
...
void	C_SkinObj::Draw(CMeshSkin& mesh)
{
...
bool    C_SkinObj::Render()
{
	PrePender();
	for (int iMesh = 0; iMesh < m_Loader.m_ObjList.size(); iMesh++)
	{
		int iSubMesh = m_Loader.m_ObjList[iMesh].subMeshSkin.size();
		if (iSubMesh > 0)
		{
			for (int iSub = 0; iSub < iSubMesh; iSub++)
			{
				Draw(m_Loader.m_ObjList[iMesh].subMeshSkin[iSub]);
			}
		}
		else
		{
			Draw(m_Loader.m_ObjList[iMesh]);
		}
	}
	return true;
}
bool    C_SkinObj::Release()
{
	C_Model::Release();
	m_Loader.Release();
	return true;
}
void	C_SkinObj::CreateVIData(CMeshSkin& mesh)
{
	HRESULT hr = S_OK;
...
void C_SkinObj::PrePender()
{
	UINT offset = 0;
	UINT stride = m_obj.m_VertexSize;
	m_obj.m_pContext->IASetVertexBuffers(0, 1, &m_obj.m_pVertexBuffer, &stride, &offset);			//vertexbuffer를 셋팅, 정점버퍼 시작슬롯, 갯수, 주소, 배열의 주소, 오프셋 배열거리
	m_obj.m_pContext->IASetIndexBuffer(m_obj.m_pIndexBuffer, DXGI_FORMAT_R32_UINT, 0);			//인덱스 버퍼 셋팅 인덱스 버퍼는 여러 버퍼를 넘기는게 아니라 버퍼s가 아니고 주소를 보낼 필요도 없다
