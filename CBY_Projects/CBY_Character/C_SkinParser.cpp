...
int  C_SkinParser::GetParentIndex(C_STR szParentName)
{
	for (int iObj = 0; iObj < m_ObjList.size();
		iObj++)
	{
...
bool C_SkinParser::Load(const TCHAR* szFileName)
{
	...
	for (int iObj = 0; iObj < m_Scene.iNumMesh; iObj++)
	{
		...
		if (m_ObjList[iObj].subMeshSkin.size() > 0)
		{
			...
			{
				_fgetts(m_pBuffer, 256, m_pStream);
				_stscanf(m_pBuffer, _T("%s %d %d %d %d %d"),
					m_pString,
					&iNumArray[0],
					&iNumArray[1],
					&iNumArray[2],
...
BOOL  C_SkinParser::C_SkinParser::OpenStream(const TCHAR* filename)
{
...
BOOL  C_SkinParser::CloseStream()
{
	fclose(m_pStream);
	return TRUE;
}

bool C_SkinParser::Release()
{
	m_ObjList.clear();
