...
LRESULT CBYCore::WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (m_pMainCamera != nullptr)
	{
		m_pMainCamera->MsgProc(hWnd, message, wParam, lParam);
	}
	return CBYWindow::WndProc(hWnd, message, wParam, lParam);
}
...
bool  CBYCore::CoreInit()
{
	...
	IDXGISurface*  pSurface;
	m_pSwapChain->GetBuffer(0, __uuidof(IDXGISurface), (void**)&pSurface);
	m_Font = std::make_shared<C_DXWrite>();
	m_Font->Set(g_hwnd, 0, 0, pSurface);
	pSurface->Release();
	//////////
	m_DebugLine = std::make_shared<CShapeLine>();
...
bool  CBYCore::CoreFrame()
{
	...
	{
		CDX::ApplyRS(m_pContext, CDX::CDxState::g_pRSWireFrame);
	}
