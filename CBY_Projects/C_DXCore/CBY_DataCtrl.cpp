...
bool CBY_DataCtrl::Save(const TCHAR* pszSave)
{
	...
	_ftprintf(fp, _T("%d %d %d %d %d %d %d %d\n"), m_data.m_CharType, m_data.SknNameList.size(),														//애니 타입, 스킨, 매트릭스, State, 
		m_data.MtrList.size(), m_data.dwStateNum, m_data.ColBoxList.size(), m_data.ObjList.size() , m_data.m_dwWeapon, m_data.m_dwCameraSocket);		//충돌박스, obj, 무기, 카메라
	...
	if (m_data.ObjList.size() > 0)
	{
		for (DWORD dw = 0; dw < m_data.ObjList.size(); dw++)
		{
			...
			_ftprintf(fp, _T("%f %f %f\n"), m_data.ObjList[dw].vpos.x, m_data.ObjList[dw].vpos.y, m_data.ObjList[dw].vpos.z);

			_ftprintf(fp, _T("%f %f %f\n"), m_data.ObjList[dw].vRot.x, m_data.ObjList[dw].vRot.y, m_data.ObjList[dw].vRot.z);

			_ftprintf(fp, _T("%f\n"), m_data.ObjList[dw].fScale);
		}
	}
	else if(m_data.m_dwWeapon > 0)  //ObjectList가 없고 무기가 있을 시
	{
		_ftprintf(fp, _T("%d\n"), m_data.m_dwFireSocket);
	}
...
bool CBY_DataCtrl::Load(const TCHAR* pszLoad)
{
	...
	_stscanf_s(pBuffer, _T("%d%d%d%d%d%d%d%d"), &m_data.m_CharType, &sknsize, &mtrsize, &m_data.dwStateNum, &boxsize, &objsize,
		&m_data.m_dwWeapon, &m_data.m_dwCameraSocket);
	...
	if (objsize > 0)
	{
		for (DWORD dw = 0; dw < objsize; dw++)
		{
			...
			_fgetts(pBuffer, 256, fp_src);
			_stscanf_s(pBuffer, _T("%f%f%f"), &m_data.ObjList[dw].vRot.x, &m_data.ObjList[dw].vRot.y, &m_data.ObjList[dw].vRot.z);
			
			_fgetts(pBuffer, 256, fp_src);
			_stscanf_s(pBuffer, _T("%f"), &m_data.ObjList[dw].fScale);
		}
	}
	else if (m_data.m_dwWeapon>0)
	{
		_fgetts(pBuffer, 256, fp_src);
		_stscanf_s(pBuffer, _T("%d"), &m_data.m_dwFireSocket);
	}
