...
bool CBY_Object::Frame()
{
	...
	if (m_dwAniType == CHAR_FRAMETYPE)
	{
		...
		{
			m_Bone->Update(m_iStartFrame, m_iEndFrame, m_fElapseTick, m_pMatrixList);
		}
	}
...
void CBY_Object::BoneBoxRender(CBY_MeshSkin* obj, int index)
{
	...
	if (obj->m_BoneBox.m_obj.m_pContext!=nullptr)
	{
		obj->m_BoneBox.SetMatrix(&(mat * world), &m_matView, &m_matProj);
