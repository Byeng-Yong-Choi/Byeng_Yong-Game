...
bool CBY_Select::PickCheck(float* pft, D3DXVECTOR3 v0, D3DXVECTOR3 v1, D3DXVECTOR3 v2)
{
	float t, u, v;
...
bool CBY_Select::ChkOBBToRay(CBY_Box* pbox)
{
...
bool CBY_Select::OBBToRay(CBY_Box* pBox)
{
	float t_min = -999999.0f;
	float t_max = 999999.0f;
	float  f[3], fa[3], s[3], sa[3];
...
void CBY_Select::SetMarix(D3DXMATRIX* pWorld, D3DXMATRIX* pView, D3DXMATRIX* pProj)
{
	if (pWorld != nullptr)
	{
		m_World = *pWorld;
...
D3DXVECTOR3 CBY_Select::GetIntersection(float t)
{
	D3DXVECTOR3 inter;
