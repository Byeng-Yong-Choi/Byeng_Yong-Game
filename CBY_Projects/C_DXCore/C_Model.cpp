...
bool C_Model::GameDataLoad(const TCHAR* pszLoad)
{
	TCHAR pBuffer[256] = { 0 };
	TCHAR pTemp[256] = { 0 };
...
void C_Model::Convert(std::vector<PT_VERTEX>& list)
{
...
void C_Model::Convert(std::vector<PNCT_VERTEX>& list)
{
	for (int i = 0; i < list.size(); i++)
	{
...
void C_Model::UVConvert(std::vector<PNCT_VERTEX>& vertex,RECT& Rect, int Width, int Height)
{
...
void C_Model::UpdateVertexData(std::vector<PNCT_VERTEX>& list)
{
	int size = list.size();
	m_VerTex.clear();
...
void C_Model::UpdatePSShader(ID3D11ShaderResourceView* textview)
{
