...
void    C_SkinObj::ConvertSkin(std::vector<PNCT2IW_VERTEX>& list)
{
	int iBaseVertex = 0;
	int iBaseIndex = 0;
	int iBaseOffset = 0;
			...
			for (int iSubMesh = 0; iSubMesh < mesh->subMeshSkin.size(); iSubMesh++)
			{
				if (iRef >= 0)
				{
					...
					for (int i = 0; i < m_Loader.m_MtlList[iRef].SubMaterial[iSubMesh].texList.size(); i++)
					{
						...
						case 2:
						{
							mesh->subMeshSkin[iSubMesh].m_pAOTexture =
								I_Texture.GetPtr(mesh->subMeshSkin[iSubMesh].m_iTexIndex);
						}
						break;
		...
		else
		{
			if (iRef >= 0)
			{
				...
				for (int i = 0; i < m_Loader.m_MtlList[iRef].texList.size(); i++)
				{
					...
					case 2:
					{
						mesh->m_pAOTexture = I_Texture.GetPtr(mesh->m_iTexIndex);
					}
					break;
...
void	C_SkinObj::Draw(CBY_MeshSkin* mesh)
{
	...
	if (mesh->m_pTexture != nullptr)
	{
		...
		if (mesh->m_pAOTexture != nullptr)
			m_obj.m_pContext->PSSetShaderResources(2, 1,
				&mesh->m_pAOTexture->m_pTextureRV);
		...
		if(mesh->m_pEmsTexture!=nullptr)
			m_obj.m_pContext->PSSetShaderResources(4, 1,
				&mesh->m_pEmsTexture->m_pTextureRV);
		if(mesh->m_pDetailTexture != nullptr)
			m_obj.m_pContext->PSSetShaderResources(5, 1,
				&mesh->m_pDetailTexture->m_pTextureRV);
	}
	if (mesh->m_iNumIndex > 0)
	{
...
bool    C_SkinObj::Release()
{
	...
	m_VertexDataSkin.clear();
	return true;
}
void	C_SkinObj::CreateVIData(CBY_MeshSkin* mesh)
{
	HRESULT hr = S_OK;
...
void C_SkinObj::VertexDataTan(CBY_MeshSkin* mesh)
{
...
void C_SkinObj::CreateTangentSpaceVectors(D3DXVECTOR3 *v0,
