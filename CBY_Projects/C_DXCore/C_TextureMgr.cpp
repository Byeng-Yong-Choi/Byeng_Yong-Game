...
INT C_TextureMgr::Add(ID3D11Device* pDevice, const TCHAR* pFileName)
{
...
INT	C_TextureMgr::Add(ID3D11Device*	 pDevice, const TCHAR* pFileName, const TCHAR* szPath)
{
	TCHAR szFileName[MAX_PATH];
	TCHAR Drive[MAX_PATH];
	TCHAR Dir[MAX_PATH];
	...
	pPoint->SetPath(szPath);
	HRESULT hr = S_OK;
	hr = pPoint->Load(pDevice, szFileName,bTga);
...
CTexture* const	C_TextureMgr::GetPtr(INT iIndex)
{
...
CTexture* const C_TextureMgr::GetPtr(T_STR strFindName)
{
	for (TEMPLATE_MAPitor itor = m_Map.begin(); itor != m_Map.end(); itor++)
	{
		CTexture* pPoint = (CTexture*)(*itor).second;
