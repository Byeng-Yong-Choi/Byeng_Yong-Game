...
namespace JH {
	...
	INT	JH_MapMgr::AddMap(const TCHAR* LoadFile, const TCHAR* LightFile)
	{
		FILE* fp;
		TCHAR  szFile[256] = { 0, };
	...
	/*	for (auto List:m_GMDataList )
		...
		LoadMap(GMData->m_pMap.get(), GMData->m_pQuadTree.get(), m_MapData.iRow, m_MapData.iCol, m_MapData.iCellCount, m_MapData.iCellSize,
			m_MapData.m_BaseTextureFile.c_str(), m_MapData.m_ShaderFile.c_str(), LightFile, m_MapData.m_NormalMapFile.c_str(), HeightFile);
