...
bool KG_Core::GameRun()
{
	...
	while (GetGameRun)
	{
		...
		else
		{
			...
			fDeltaTime = ((float)dwCurrentTime - (float)m_dwOldTime)/1000.0f;
			m_dwOldTime = dwCurrentTime;
			m_fGameFrameTime += fDeltaTime;

			//if (m_fGameFrameTime>0.016f)
			{
				if (CoreFrame() == false)
				{
					return false;
...
bool  KG_Core::CoreInit()
{
	...
	//global
	g_camera = m_pMainCamera;
	g_device = m_pd3dDevice;
	g_context = m_pContext;
	g_swapChain = m_pSwapChain;
...
void KG_Core::Debug()
{
	...
	rt = Winrt;
	swprintf_s(text, L"FPS : %d, intersection : (%0.2f,%0.2f,%0.2f)", g_FPS, g_CharPos.x, g_CharPos.y, g_CharPos.z);
